...
export async function POST(request: Request) {
  ...
  if (session.user.role !== "ADMIN") {
    return NextResponse.json({ message: "Hanya Admin yang bisa membuat produk" }, { status: 403 });
  }
  ...
  if (!parsed.success) {
    return NextResponse.json(
      { message: parsed.error.issues[0]?.message ?? "Input produk tidak valid" },
      { status: 400 }
    );
  }

  if (image instanceof File && (!allowedImageTypes.includes(image.type) || image.size > maxImageSize)) {
